...
class Predictor:
    ...
    def predict(self, image: np.ndarray, with_other=False, expected_angle: float | None = None, expected_angle_diff: float | None = None):
        if image.ndim == 3:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=torch.cuda.is_available()):
                if expected_angle is not None:
                    forward_kwargs = {'expected_angle': expected_angle, 'expected_angle_diff': expected_angle_diff, 'suppress_angle': True}
                else:
                    forward_kwargs = {}
